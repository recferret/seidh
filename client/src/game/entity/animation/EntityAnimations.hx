...
class EntityAnimation {
    ...
    private var entityAnimationState:EntityAnimationState;
    ...
    public function new(parent:h2d.Object) {
        ...
        animation.onAnimEnd = function callback() {
            if (entityAnimationState != DEAD) {
                if (entityAnimationState != RUN && entityAnimationState != WALK) {
    ...
    public function setAnimationState(newState:EntityAnimationState) {
        if (newState != entityAnimationState || sideChanged) {
            sideChanged = false;
            entityAnimationState = newState;
            hideAnimations();
